...
| --- | --- | --- |
| **Import Data…** | Ctrl+O | Opens a file dialog and imports the chosen file with **Auto Loader**. It records a *File Loader* row (plus the suggested roles) in the protocol. To choose a specific loader, use the **Data Loader** menu in the [Data Importer](UI-Data-Importer) panel instead. |
| **Import Folder…** | — | Opens a folder dialog and shows *"Folder selected: <name>"* in the status bar. It does not load files. To process every file in a folder, use [Run Sequence](UI-Run-Sequence). |
...
| **Export Sequence.py…** | Ctrl+S | Saves the protocol as a runnable Python file (default `config/sequences/sequence.py`). See [Bulk Runs and Headless Use](Bulk-Runs-and-Headless). |
...
![About PhysPlot dialog](images/ui/ui_about_dialog.png)
...
![Status bar in Simple Mode](images/ui/ui_status_simple.png)
...
![Status bar in Advanced Mode](images/ui/ui_status_advanced.png)
